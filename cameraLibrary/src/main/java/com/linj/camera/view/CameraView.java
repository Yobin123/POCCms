...
//                mCamera = Camera.open();//开启相应的摄像头
                setCameraParameters();//设置相应的参数
                mCamera.setPreviewDisplay(holder);
                mCamera.startPreview();//设置相应的预览
            } catch (Exception e) {
    ...
    private boolean openCamera() {
        if (mCamera != null) {//每次open一次都必须释放一下相应camera
            mCamera.stopPreview();
            mCamera.release();
            mCamera = null;
        }
        ...
        if (mIsFrontCamera) {
            Camera.CameraInfo cameraInfo = new CameraInfo();//获取摄像的个数
